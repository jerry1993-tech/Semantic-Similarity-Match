...
# Modified by Hou and tests all passed.
# Added Lists:
#   1 Add `learning rate multiplie` in 29/9/2021.
#   2 Add `padding mask` in 08/10/2021.
#   3 Add `F1 metrics` in `metric.py` in 08/10/2021.
#   ...
# ==============================================================================
...
class CRF(tf.keras.layers.Layer):
    ...
    def __init__(self,
        ...
        # Add crf label2idx_map
        self.label2idx_map = label2idx_map
        
        # Add mcrf
        #  References:
        # [Masked Conditional Random Fields](https://arxiv.org/abs/2103.10682)
        self.use_mcrf = use_mcrf
    ...
    def build(self, input_shape):
        ...
        if self.lr_multiplier != 1:
            K.set_value(self.chain_kernel, K.eval(self.chain_kernel) / self.lr_multiplier)
    ...
    def call(self, inputs, mask=None, text=None, **kwargs):
        # mask: Tensor(shape=(batch_size, sequence_length), dtype=bool) or None
        if self.supports_masking:
            mask = tf.cast(tf.not_equal(text, 0), tf.int32)
        if mask is not None:
    ...
    def trans(self):
        if self.mask_tran_matrix is not None:
            self.chain_kernel = tf.minimum(self.chain_kernel, self.mask_tran_matrix)
        if self.lr_multiplier != 1:
            return self.lr_multiplier * self.chain_kernel
        else:
    ...
    def get_viterbi_decoding(self, potentials, sequence_length):
        ...
        decode_tags, best_score = crf_decode(potentials, self.trans,
                                             sequence_length)

        return decode_tags, best_score

    """TODU: id and str mapping -> DONE."""
    ...
    def get_negative_log_likelihood(self, y_true):
        ...
        """
        if self.mask_tran_matrix is not None:
            self.trans = tf.minimum(self.trans, self.mask_tran_matrix)
        """
        ...
        log_likelihood, _ = crf_log_likelihood(
            self.potentials, y_true, self.sequence_length, self.trans)
    ...
    def _dense_layer(self, input_):
        ...
        return tf.keras.backend.cast(output, self.trans.dtype)
